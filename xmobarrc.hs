Config { font = "xft:JetBrains Mono:style=Bold:pixelsize=11:antialias=true:hinting=true"
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " <icon=haskell.xpm/><fc=#d7d7d7> ==</fc> %UnsafeStdinReader% }{ <fc=#99C794><fn=3></fn> %uname%</fc>    <fc=#ffff5f>%cpu%</fc>   <fc=#ff5f87>%memory%</fc>   <fc=#87afd7>%disku%</fc>   <fc=#5af78e>%battery%</fc>   <fc=#ff79c6>%uptime%</fc>   %dynnetwork%   <fc=#afd75f>%date%</fc>  "
       }         
