Config { font = "xft:JetBrains Mono:style=Bold:pixelsize=12:antialias=true:hinting=true"
       ...
       , fgColor = "#ffffff"
       --, alpha   = 200
       , position = Static { xpos = 3, ypos = 3, width = 1910, height = 30 }
       , lowerOnStart = True 
       , hideOnStart = False
       , allDesktops = True
       , persistent = True
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " <icon=haskell_20.xpm/><fc=#d7d7d7> ==</fc> %UnsafeStdinReader% }{ <fc=#99C794><fn=3></fn> %uname%</fc>    <fc=#ffff5f>%cpu%</fc>   <fc=#ff5f87>%memory%</fc>   <fc=#87afd7>%disku%</fc>   <fc=#5af78e>%battery%</fc>   <fc=#ff79c6>%uptime%</fc>   %dynnetwork%   <fc=#afd75f>%date%</fc>  "
       }         
